...
export default function GameDetails({ params }) {
  const { id } = use(params);
  const { games, isLoaded, buyGame, deleteGame } = useGames();
  const { user } = useAuth();
        ...
        <div className="flex-[2_2_0%]">
          ...
          <div className="mt-8 flex flex-wrap gap-4">
            <button 
              ...
              className={`px-5 py-2.5 text-white border-none cursor-pointer font-bold text-base rounded-sm transition-colors shadow-sm ${game.isPurchased ? "bg-gray-500 cursor-not-allowed" : "bg-[#28a745] hover:bg-[#218838]"}`}
            >
            ...
            {user && user.uid === game.ownerId && (
              <>
                ...
                <button 
                  ...
                  className="px-5 py-2.5 bg-red-600 text-white border-none cursor-pointer font-bold text-base rounded-sm hover:bg-red-700 transition-colors shadow-sm"
                >
                  Usuń pozycję
                </button>
              </>
