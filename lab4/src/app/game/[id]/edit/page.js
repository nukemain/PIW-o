...
export default function EditGamePage({ params }) {
  ...
  if (!user || user.uid !== gameToEdit.ownerId) {
    return (
      <div className="w-full max-w-[800px] mx-auto p-10 text-center">
        <h2 className="text-2xl font-bold mb-4 text-red-600">Brak uprawnień</h2>
        <p className="mb-4">Możesz edytować tylko gry dodane przez siebie.</p>
        <Link href={`/game/${id}`} className="text-[#0056b3] hover:underline">Wróć do szczegółów gry</Link>
      </div>
    );
  }
